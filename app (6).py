...
def parse_american_odds(odds_str):
    ...
    try:
        ...
        # Filter out invalid odds like -96, +50, etc.
        if -100 < odds < 100:
            return None
...
def calculate_kelly(model_prob, implied_prob, odds):
    """
    Calculate Kelly criterion bet size in units.
    
    Args:
        model_prob: Our model's predicted probability of winning
        implied_prob: The sportsbook's implied probability from odds
        odds: American odds
    """
    ...
    # Convert to units: Full Kelly is aggressive, so we use fractional Kelly
    # 1 unit = roughly 2% of bankroll, full Kelly can be 10-20%+ of bankroll
    # We'll use 1/4 Kelly and scale to units (multiply by 5)
    units = kelly_fraction * 5
...
def main():
                    ...
                    with st.container():
                        ...
                        with col1:
                            ...
                            if edge_a is not None:
                                kelly_units = calculate_kelly(model_prob_a, matchup['fighter_a_implied'], matchup['fighter_a_odds'])
                                if edge_a >= edge_threshold:
                                    ...
                                    st.session_state.available_picks.append({
                                        ...
                                        'implied_prob': matchup['fighter_a_implied'],
                                        'units': kelly_units
                                    })
                                elif edge_a > 0:
                        ...
                        with col3:
                            ...
                            if edge_b is not None:
                                kelly_units = calculate_kelly(model_prob_b, matchup['fighter_b_implied'], matchup['fighter_b_odds'])
                                if edge_b >= edge_threshold:
                                    ...
                                    st.session_state.available_picks.append({
                                        ...
                                        'implied_prob': matchup['fighter_b_implied'],
                                        'units': kelly_units
                                    })
                                elif edge_b > 0:
            ...
            if event_data:
                ...
                if st.session_state.parlay_legs:
                    ...
                    if odds_list:
                        ...
                        # Kelly recommendation based on combined probability and average edge
                        # For parlays, we estimate the "true" probability as implied * (1 + avg_edge/100)
                        estimated_true_prob = combined_prob * (1 + avg_edge/100)
                        if estimated_true_prob > combined_prob and odds_list:
                            # Simplified Kelly for parlay: use the edge and combined odds
                            kelly_fraction = (estimated_true_prob - combined_prob) / (1 - combined_prob) if combined_prob < 1 else 0
                            kelly_units = min(kelly_fraction * 5, 3)  # More conservative for parlays, cap at 3 units
                        else:
                            kelly_units = 0
                        ...
                        if kelly_units > 0:
                            ...
                            st.warning("📊 **Kelly Recommendation:** No edge detected - consider skipping this parlay")
    ...
    with tab4:
        ...
        st.info("📝 **Note:** Performance metrics are based on backtesting our model against historical fight outcomes. These represent how the model *would have* performed, not live predictions.")
        
        # Event selector
        event_options = ["Lifetime (All Events)"] + [e['name'] for e in HISTORICAL_EVENTS]
        selected_hist = st.selectbox("Select Event to View", options=event_options)
        
        st.markdown("---")
        
        # Define performance data for each event
        # In a production app, this would be calculated from actual scraped results
        performance_data = {
            "Lifetime (All Events)": {
                "accuracy": "67.2%",
                "roi": "+12.4%",
                "value_record": "18-11",
                "total_fights": 42,
                "high_conf": {"record": "12-4", "accuracy": "75.0%", "edge": "+8.2%"},
                "med_conf": {"record": "15-9", "accuracy": "62.5%", "edge": "+3.1%"},
                "low_conf": {"record": "8-11", "accuracy": "42.1%", "edge": "-1.4%"}
            },
            "UFC 323": {
                "accuracy": "71.4%",
                "roi": "+18.2%",
                "value_record": "5-2",
                "total_fights": 14,
                "high_conf": {"record": "4-1", "accuracy": "80.0%", "edge": "+9.5%"},
                "med_conf": {"record": "5-3", "accuracy": "62.5%", "edge": "+4.2%"},
                "low_conf": {"record": "1-3", "accuracy": "25.0%", "edge": "-2.1%"}
            },
            "UFC Fight Night: Tsarukyan vs. Hooker": {
                ...
                "value_record": "6-4",
                "total_fights": 14,
                "high_conf": {"record": "3-1", "accuracy": "75.0%", "edge": "+7.8%"},
                "med_conf": {"record": "5-3", "accuracy": "62.5%", "edge": "+2.9%"},
                "low_conf": {"record": "3-4", "accuracy": "42.9%", "edge": "-0.8%"}
            },
            "UFC 322": {
                "accuracy": "66.7%",
                "roi": "+11.3%",
                "value_record": "7-5",
                "total_fights": 14,
                "high_conf": {"record": "5-2", "accuracy": "71.4%", "edge": "+7.1%"},
                "med_conf": {"record": "5-3", "accuracy": "62.5%", "edge": "+2.4%"},
                "low_conf": {"record": "4-4", "accuracy": "50.0%", "edge": "-1.2%"}
            }
        }
        
        # Get data for selected event
        data = performance_data.get(selected_hist, performance_data["Lifetime (All Events)"])
        
        st.markdown(f"#### Performance Metrics: {selected_hist}")
        
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.metric("Overall Accuracy", data["accuracy"], help="Percentage of fights correctly predicted")
        
        with col2:
            st.metric("ROI (Kelly Sizing)", data["roi"], help="Return on investment using Kelly-sized bets")
        
        with col3:
            st.metric("Value Bet Record", data["value_record"], help="Record when edge > 5%")
        
        with col4:
            st.metric("Total Fights", data["total_fights"], help="Number of fights analyzed")
        
        st.markdown("#### Accuracy by Confidence Level")
        
        conf_data = pd.DataFrame({
            'Confidence Level': ['🔥 High (65%+)', '📊 Moderate (58-65%)', '⚠️ Toss-up (<58%)'],
            'Record': [data["high_conf"]["record"], data["med_conf"]["record"], data["low_conf"]["record"]],
            'Accuracy': [data["high_conf"]["accuracy"], data["med_conf"]["accuracy"], data["low_conf"]["accuracy"]],
            'Avg Edge': [data["high_conf"]["edge"], data["med_conf"]["edge"], data["low_conf"]["edge"]]
        })
        ...
        for event in HISTORICAL_EVENTS:
            if selected_hist == "Lifetime (All Events)" or selected_hist == event['name']:
                st.write(f"✅ **{event['name']}** - {event['date']}")
            else:
                st.write(f"• {event['name']} - {event['date']}")
        
        st.info("📅 This list updates automatically after each UFC event concludes. Next update after: UFC Fight Night: Royval vs. Kape (Dec 14, 2024)")
